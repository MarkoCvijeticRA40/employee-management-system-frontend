...
@Component({
  selector: 'app-hr-manager-edit-profile',
  templateUrl: './hr-manager-edit-profile.component.html',
  styleUrls: ['./hr-manager-edit-profile.component.css']
})
export class HrManagerEditProfileComponent implements OnInit {
  ...
  public ChangePassword() {
    this.router.navigate(['/hrmanager/change-password']);
  }
  ...
  public updateUser() {
    if (this.isEmpty(this.currentUser.email) ||
      ...
      this.userService.editHrManagerAccount(this.currentUser.id,this.currentUser).subscribe(res => {
        this.currentUser = res;
